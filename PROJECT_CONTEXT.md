...
**Olila Glass** is an in-browser retail POS + inventory app for a Bangladeshi ceramic/tableware shop.

- Users: shop owner / counter staff (demo auth; always `role: 'admin'`, name `"Shop Owner"`).
- Currency: BDT (`৳`).
...
There is **no backend**. All state lives in React context and resets on refresh.
...
**Not used in app code today:** `html2canvas`, `jspdf`. No React Router, no Firebase, no env API.
...
[`src/context/AppContext.tsx`](src/context/AppContext.tsx) — `useReducer` + `AppProvider`.

- Seed: `masterProducts` (604 SKUs).
- Empty on boot: `sales`, `inventoryLogs`, `cart`.
...
Sale { id, date, total_amount, customer_name?, customer_phone?, items }
...
Source of truth: [`src/data/masterProducts.ts`](src/data/masterProducts.ts)  
Generated from `olila master product list.xlsx` — **do not hand-edit row-by-row**; regenerate if the Excel changes.
...
|--------|------|----------|
| Login | `LoginPage.tsx` | Any non-empty user + password ≥4 chars; demo fill `olila` / `root` |
| Dashboard | `Dashboard.tsx` | Catalog count, sales count, OOS, today’s revenue; charts; Open POS |
| Products | `ProductsPage.tsx` | Search/filter (group/category), CRUD, CSV export |
...
3. Prefer editing existing pages/context over new frameworks. Do not add a backend unless explicitly asked.
4. Do not commit `Ref web desine/` (local UI reference screenshots) unless the user asks.
5. Keep `masterProducts.ts` as generated catalog data; if regenerating from Excel, keep mapping in §4.
...
- [ ] No persistence (refresh wipes sales, stock edits, login)
- [ ] Auth is demo only; roles unused for ACL
- [ ] Payment type / paid amount not stored on `Sale`
- [ ] `customer_phone` unused from POS
- [ ] README / SETUP_GUIDE still mention outdated ShopEase/Firebase wording
